...
Step 3에서 `useFlag()`를 호출하는 순간, 아래 정보가 ExperiBase에 자동으로 전송됩니다.
...
Step 4처럼 `sdk.track()`을 호출할 때만 아래 정보가 전송됩니다.
