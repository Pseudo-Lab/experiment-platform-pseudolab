...
Cloudflare D1
  pseudolab-main         → 커뮤니티 데이터 (Discord, GitHub)
                          + 성장시스템 dl_* 테이블 (매일 동기화, 2026-04-04~)
                            dl_profiles (8,052건), dl_projects (3,504건),
                            dl_project_members (3,658건), dl_project_attendance (751건) 등
                          ※ dl_xp_history, dl_user_roles 는 현재 0건 (파이프라인 미연결)
  pseudolab-exp          → 실험 플랫폼 메타데이터 (읽기/쓰기)
                            experiments, experiment_variants, experiment_assignments ✅

실험 분석                 → pseudolab-exp (할당 기록)
                           + pseudolab-main dl_* 피처 테이블 (결과 지표)
...
| 지표명 | 설명 | D1 소스 | 현재 데이터 |
|---|---|---|---|
| `xp_gain_7d` | 7일간 XP 획득량 | `dl_xp_history` | ⚠️ 0건 (파이프라인 미연결) |
| `project_attendance_rate` | 프로젝트 세션 출석률 | `dl_project_attendance` | ✅ 751건 |
| `project_completion_rate` | 프로젝트 완료율 | `dl_projects`, `dl_project_members` | ✅ 3,504 / 3,658건 |
| `quest_completion_rate` | 퀘스트 완료율 | `dl_user_quests` | ✅ 10건 |
| `badge_acquisition_rate` | 뱃지 획득률 | `dl_user_badges` | ✅ 14건 |
| `retention_d7` | 7일 리텐션 | `dl_xp_history` 기반 추정 | ⚠️ 0건 (파이프라인 미연결) |
...
| 기준 | D1 소스 테이블 | 현재 데이터 |
|---|---|---|
| 프로젝트 참여 여부 | `dl_project_members` | ✅ 3,658건 |
| 출석률 | `dl_project_attendance` | ✅ 751건 |
| 역할 | `dl_user_roles` | ⚠️ 0건 |
| XP 획득량 | `dl_xp_history` | ⚠️ 0건 |
| 뱃지 보유 | `dl_user_badges` | ✅ 14건 |
| 레벨/포인트 | `dl_profiles` | ✅ 8,052건 |
...
| 실험 상태 전환 유효성 검사 | ✅ 완료 (2026-04-15) |
| 성장시스템 데이터 D1 연동 확인 | ✅ 확인 완료 (2026-04-15) |
| Supabase Auth 연동 | 🟡 보류 (POC 단계, 추후 도입) |
...
- [x] 실험 상태 전환 유효성 검사 (2026-04-15 완료)
- [x] pseudolab-main dl_* 성장시스템 데이터 확인 (2026-04-15)
- [ ] 성장시스템 데이터 → 실험 결과 지표 연결 API 구현
- [ ] 분석가 EDA 완료 후 → 지표 카탈로그 공동 작성
- [ ] 실험 설계 확정 후 → 세그멘테이션 모듈 개발
- [ ] Supabase Auth 연동 (POC 이후, 역할 기반 접근 제어)
