...
초기 요청서(`req.md`)는 “LVUP 프로젝트 상세 홈에 12기 회고 배너를 띄울지 판단하는 API”를 요구했다. 작업 중 사용자 피드백과 플랫폼 아키텍처 검토를 거치면서, 최종 구현은 배너 전용 API가 아니라 `placement_key` 중심의 `experiment placement` API로 정리했다.
...
3. 실험 플랫폼 대시보드 프론트엔드에는 placement config 생성/조회/편집/삭제 UI가 붙었다.
   - 실험 생성 화면에서 초기 LVUP 노출 슬롯을 선택적으로 함께 생성할 수 있다.
