...
외부 LVUP 프론트엔드가 12기 프로젝트 회고 진입 UI를 노출할지 실험 플랫폼에 물어볼 수 있도록, 실험별 전용 라우트가 아니라 범용 `experiment placement` decide/config API를 백엔드에 추가했다.
...
다만 이 구조는 향후 정식 실험으로 전환할 때 버릴 임시 API가 아니다. `experiments/{experiment_id}/placements/{placement_key}` 구조는 그대로 두고, 플랫폼 기능이 완성되면 placement decide 내부에서 assignment, feature flag rule, exposure logging을 연결할 수 있다. 즉, 이번 PR은 과도기 준실험을 안정적으로 집행하면서도 정식 실험 플랫폼 흐름으로 확장 가능한 하위 리소스를 추가하는 작업이다.
...
- 라우트가 `/experiments/{experiment_id}/placements/{placement_key}` 아래에 있어 기존 experiment 중심 모델과 맞다.
...
2. `{experiment_id}` 실험이 존재하는지 확인한다.
3. `{placement_key}` 설정이 존재하는지 확인한다.
4. 실험 상태가 `running`이고 placement가 enabled인지 확인한다.
5. 해당 프로젝트가 target cohort, 현재는 12기 프로젝트인지 확인한다.
6. 해당 사용자가 프로젝트 멤버인지 확인한다.
7. 프로젝트 role이 `builder` 또는 `runner`인지 확인한다.
8. 멤버십 상태가 `active`인지 확인한다.
9. 현재 시간이 회고 노출 기간 안인지 확인한다.
10. `user_id + experiment_id` 기준으로 이미 회고를 제출했는지 확인한다.
...
외부 LVUP 프론트엔드는 `user_id`, `project_id`만 보내면 되고, 실험 플랫폼은 D1 동기화 데이터와 실험 설정을 기준으로 노출 여부를 판단한다. 현재 UI는 `ui.type = "banner"`로 내려가지만, 나중에 카드/모달/CTA로 바뀌어도 API 구조는 유지된다.
